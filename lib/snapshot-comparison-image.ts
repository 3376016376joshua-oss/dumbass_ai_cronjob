...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">
  ...
  <text x="92" y="126" fill="#f1fff8" font-size="34" font-family="Arial, sans-serif" font-weight="800">AI Stupid Meter Coding Overlay</text>
  <text x="92" y="172" fill="#89a39b" font-size="18" font-family="Arial, sans-serif">Fetched ${escapeXml(fetchedAt)}</text>
  <text x="92" y="284" fill="#f1fff8" font-size="30" font-family="Arial, sans-serif" font-weight="700">No comparison data available</text>
</svg>`;
}
...
export async function renderSnapshotComparisonPng(snapshotResult: any, period: HistoricalPeriod = '7d') {
  ...
  if (series.length === 0) {
    const empty = new Resvg(renderNoDataSvg(snapshotResult), {
...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">
  ...
  <text x="${WIDTH - 330}" y="112" fill="${winner?.color || '#6aff9e'}" font-size="24" class="sans" font-weight="800">Best avg: ${escapeXml(winner?.name || '--')}</text>
  <text x="${WIDTH - 330}" y="143" fill="#f4fff8" font-size="20" class="mono">${winner ? `${formatNumber(winner.average, 1)} average score` : '--'}</text>
